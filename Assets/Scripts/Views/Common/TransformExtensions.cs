...
public static class TransformExtensions {
	...
	// 便利メソッド
	// transform.position.x = 10f;とかできないから便利メソッド作っておく
	...
	/// 名前一致で子要素を探索
	/// </summary>
	/// <returns>The children by name.</returns>
	/// <param name="trans">Trans.</param>
	/// <param name="name">Name.</param>
	public static Transform GetChildrenByName(this Transform trans, string name) {
		foreach (Transform child in trans) {
			if (child.name == name){
				return child;
			}
			Transform t = child.GetChildrenByName (name);
			if (t != null) {
				return t;
			}
		}
